...
import SkeletonTransactionTable from "./SkeletonTable";
...
const TransactionTable = ({ className }: TransactionTablePropType) => {
	const { isLoading, wallets, currentWalletIndex } = useContext(
	...
	if (isLoading) {
		return <SkeletonTransactionTable />;
	} else if (!isLoading && (!wallets || wallets.length === 0)) {
